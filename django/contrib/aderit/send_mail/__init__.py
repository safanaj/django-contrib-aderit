...
# pylint: disable-msg=C0103,W0142,R0912,R0914,R0915,E1101

# django.contrib.aderit.send_mail -- python module to manage mails
#
...
def _SendTypeMail(kwargs, in_bulk=False):
    ...
    Il dizionario @kwargs è del tipo:
    {
      ...
      'email_backend': <sottoclasse di BaseEmailBackend> o 'stringa',  -- OPZ
    ...
    def _check_and_clean_mailto_list(mailto_list):
        '''check mailto to ensure delivering.'''
        _mailto = mailto_list[:]
        raise_error = True
        for idx, m in enumerate(_mailto):
            if m == "":
                logger.warning("_SendTypeMail: removed an empty email"
                               " in 'mailto' at index %d", idx)
                mailto_list.pop(mailto_list.index(m))
            else:
                raise_error = False
        if raise_error:
            raise SendTypeMailError("At least a recipient is required,"
                                    " 'mailto' passed: %s", _mailto)
    ...
    try:
        ...
        raise SendMail.DoesNotExist("requested SendMail type:"
                                    " \"%s\" does not exist" % tipo)
    ...
    smtp_host = conf_dict.pop('smtp_host',
                              getattr(settings, 'EMAIL_HOST', 'localhost'))
    smtp_port = conf_dict.pop('smtp_port',
                              getattr(settings, 'EMAIL_PORT', 25))
    smtp_user = conf_dict.pop('smtp_user',
                              getattr(settings, 'EMAIL_HOST_USER', ''))
    smtp_password = conf_dict.pop('smtp_password',
                                  getattr(settings, 'EMAIL_HOST_PASSWORD', ''))
    smtp_use_tls = conf_dict.pop('smtp_use_tls',
                                 getattr(settings, 'EMAIL_USE_TLS', False))
    email_backend = conf_dict.pop('email_backend',
                                  getattr(settings,
                                          'EMAIL_BACKEND',
                                          EmailBackend))
    if isinstance(email_backend, str):
        ...
        try:
            ...
            raise ValueError("Invalid module name "
                             "in email_backend key: %s" % mod_name)
        except ImportError:
            raise ImportError("No module named %s" % mod_name)
        try:
            email_backend = getattr(mod, class_name)
        except AttributeError:
            raise AttributeError("No class % in %s module" %
                                 (class_name, mod_name))
    if not issubclass(email_backend, BaseEmailBackend):
        raise Exception("email_backend have to be"
                        " a subclass BaseEmailBackend")
    logger.debug("Send mail %s to %s -- conf_dict_string: %s",
                 subj, mailto, str(conf_dict))
