...
TEMPLATE_NAME = getattr(settings, "NEWS_TEMPLATE_NAME", "news/news.html")
...
def get_latest_news(parser, token):
    ...
    if len(bits) == 3:
        ...
        try:
            ...
            raise template.TemplateSyntaxError("If provided, second argument"
                                               " to `get_latest_news` must be"
                                               " a positive whole number.")
    if bits[-2].lower() != 'as':
        _msg = "Missing 'as' from 'get_latest_news'"\
            " template tag. Format is "\
            "{% get_latest_news 5 as news %}."
        raise template.TemplateSyntaxError(_msg)
    return LatestNewsNode(bits[-1], limit)


@register.inclusion_tag('%s' % TEMPLATE_NAME, takes_context=True)
